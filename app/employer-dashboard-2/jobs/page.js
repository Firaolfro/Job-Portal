...
// Initial Jobs data (moved here) - Added 'id' for routing
const initialJobs = [
  {
    id: "job1",
...
export default function MyJobs() {
  ...
  const [selectedJobTitle, setSelectedJobTitle] = useState(""); // to pass job title
  ...
  const handlePromote = (index) => {
    ...
    setSelectedJobTitle(job.title); // Pass job title to modal
    setShowPromoteModal(true); // Open modal
  ...
  // Handler for View Applications button

  const handleViewApplications = (jobId, jobTitle) => {
    console.log("handleViewApplications triggered:", { jobId, jobTitle });
    router.push(
      `/employer-dashboard-2/jobs/${jobId}/applications?jobTitle=${encodeURIComponent(
        jobTitle
      )}`
    );
  };
  ...
  // const handleViewApplications = (jobId, jobTitle) => {
  //   console.log("handleViewApplications triggered:", { jobId, jobTitle });
  //   // Navigate to the applications page with jobId in the URL and jobTitle as a query parameter
  //   router.push(
  //     `/employer-dashboard-2/jobs/${jobId}/applications?jobTitle=${encodeURIComponent(
  //       jobTitle
  //     )}`
  ...
  //   setViewingApplicationsForJobId(jobId); // Keep this for conditional rendering (though navigation handles the display)
  //   setViewingApplicationsJobTitle(jobTitle); // Keep this for the back button text
        ...
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job, idx) => (
            <div
              ...
              className="grid grid-cols-my-jobs-table gap-4 items-center py-4 px-4 transition-colors duration-200 hover:bg-gray-50"
            >
              {/* Job Title & Type */}
              ...
              <div className="flex flex-col lg:flex-row items-center justify-center relative gap-2 lg:gap-4">
                <button
                  ...
                  onClick={() => handleViewApplications(job.id, job.title)} // Pass job ID and title
                >
                  View Applications
                </button>
