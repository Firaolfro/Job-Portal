    ...
    <div className="p-4 space-y-4">
      <div className="flex justify-between">
        <h3 className="font-semibold">
          Jobs Alerts <span className="text-gray-400 text-xs">(9 new jobs)</span>
        </h3>
        <div className="text-gray-600 flex justify-between items-center text-xs font-semibold">
          <BiEditAlt className="h-5 w-5" />
          <button className="hover:bg-gray-300 p-4 hover:rounded-md">Edit Job Alerts</button>
        </div>
      </div>
      {jobs.map((job, index) => (
          ...
          <div className="flex items-center space-x-4 hover:border-2 hover:border-blue-500 hover:px-5 hover:rounded-md hover:py-1">
            ...
            <div className="flex-1">
              <div className="flex items-center">
                ...
                <p className="text-xs text-blue-500 bg-blue-100 rounded-full px-3">{job.type}</p>
              </div>
              <div className="flex items-center">
                <GrLocation className="h-5 w-5 text-gray-300 pr-1" />
                <p className="text-xs text-gray-500 pr-3 font-semibold">{job.location}</p>
                <BsCurrencyDollar className="h-5 w-5 text-gray-300 pr-1" />
                <p className="text-xs text-gray-500 pr-3 font-semibold">{job.salary}</p>
                <FiCalendar className="h-5 w-5 text-gray-300 pr-1" />
                <p className="text-xs text-gray-500 font-semibold">{job.status}</p>
              </div>
            </div>
              ...
              <div className="text-right flex items-center space-x-4">
                <button onClick={() => toggleBookmark(index)}>
                  {bookmarkedJobs[index] ? (
                    <BookmarkIcon fill="black" stroke="black" className="w-5 h-5" />
                  ) : (
